...
FROM
  plant_post_v post
  JOIN plant p ON post.id = p.id
  JOIN caretaker_to_plant_count pc ON pc.id = p.care_taker_id
  JOIN plant_post po ON post.id = po.plant_id
  JOIN person seller ON seller.id = po.seller_id
WHERE
  post.id = plantId INTO res;
  RETURN res;
END
$$
LANGUAGE plpgsql;
